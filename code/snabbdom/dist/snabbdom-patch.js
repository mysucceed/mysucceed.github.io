...
function sameVnode(vnode1, vnode2) {
    //比较两个vnode节点的key和sel（节点类型的selector字符串如："div#app"）
...
function init(modules, domApi) {
    ...
    function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue) {
        ...
        var oldEndIdx = oldCh.length - 1;  //旧的节点的oldEndIdx
        
        var oldStartVnode = oldCh[0];   //旧的开始节点
        var oldEndVnode = oldCh[oldEndIdx];  //旧的结束节点

        var newEndIdx = newCh.length - 1;  //新的节点的oldEndIdx
        
        var newStartVnode = newCh[0];   //新的开始节点
        var newEndVnode = newCh[newEndIdx];   //新的结束节点
        ...
        while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
            ...
            //旧的开始节点比较新的开始节点  向中间移动一位
            else if (sameVnode(oldStartVnode, newStartVnode)) {
                patchVnode(oldStartVnode, newStartVnode, insertedVnodeQueue);
                oldStartVnode = oldCh[++oldStartIdx];
                newStartVnode = newCh[++newStartIdx];
            }
            //旧的结束节点比较新的结束节点  向中间移动一位
            ...
            //旧的开始节点比较新的结束节点
                ...
                //旧的开始节点 插在 旧的结束节点后一个位置  ---变成旧的末尾节点
            ...
            //旧的结束节点比较新的开始节点
                ...
                if (oldKeyToIdx === undefined) {
                    ...
                    //{key5: i5, key6:i6}  
                    // 旧的oldCh数组（ldStartIdx, oldEndIdx）之间遍历成对象，  obj 键是key，值 index
                }
                //新的节点的key是否存在旧的child中  （比较key）
                idxInOld = oldKeyToIdx[newStartVnode.key];
                
                //新的开始节点不在旧的child子节点中（key不同）
                if (isUndef(idxInOld)) {
                    //新的开始节点插入旧的开始节点之前  ---更新旧的开始节点
                    api.insertBefore(parentElm, createElm(newStartVnode, insertedVnodeQueue), oldStartVnode.elm);
                    newStartVnode = newCh[++newStartIdx];
                }
                else {
                //新的开始节点与旧的child中某个子节点相同（key相同）    
                    elmToMove = oldCh[idxInOld];     //相同的旧的child子节点  （对象引用地址）
                    //key相同的前提下，比较类型sel 如："div#app"
                    ...
                    //key相同的前提下，sel相同    
                        ...
                        oldCh[idxInOld] = undefined;   //旧的子节点  = undefind
                        //旧的子节点插入在旧的开始节点之前
        ...
        if (oldStartIdx > oldEndIdx) {
            ...
            //旧的子节点先遍历完    newStartIdx, newEndIdx 之间的节点插入旧child子节点的before位置  （ null 旧的子节点尾部）
            addVnodes(parentElm, before, newCh, newStartIdx, newEndIdx, insertedVnodeQueue);
        }
        //新的开始节点大于新的结束节点
        else if (newStartIdx > newEndIdx) {
            //新的子节点先遍历完   oldStartIdx, oldEndIdx 之间的旧节点删除
    ...
    function patchVnode(oldVnode, vnode, insertedVnodeQueue) {
        ...
        //新的vnode不存在text文本节点子  新的vnode节点undefined
        if (isUndef(vnode.text)) {
            //新旧的vnode都有children， 调用 updateChildren
            if (isDef(oldCh) && isDef(ch)) {
                if (oldCh !== ch)
                    updateChildren(elm, oldCh, ch, insertedVnodeQueue);
            }
            //新vnode的children节点存在，旧oldCh的children节点不存在。  添加新vnode的children
            else if (isDef(ch)) {
                if (isDef(oldVnode.text))
                    api.setTextContent(elm, '');
                addVnodes(elm, null, ch, 0, ch.length - 1, insertedVnodeQueue);
            }
            //旧vnode的children节点存在，新vnode的children节点不存在  删除旧vnode的children
            else if (isDef(oldCh)) { 
                removeVnodes(elm, oldCh, 0, oldCh.length - 1);
            }
            //新旧的vnode都没有children，新的text节点undefined，存在旧text节点
    ...
    return function patch(oldVnode, vnode) {
        ...
        if (sameVnode(oldVnode, vnode)) {
            ...
            // 没有必要去patch的。 删除oldVnode，新的vnode插入parent
            elm = oldVnode.elm;
